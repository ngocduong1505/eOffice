...
const ROLE_ACTIONS: Record<string, Set<string>> = {
  'van-thu':  new Set(['them-ho-so', 'lich-su']),
  'giam-doc': new Set(['chi-dao', 'uy-quyen', 'tu-choi', 'them-ho-so', 'lich-su']),
  'thu-ky':   new Set(['dieu-phoi', 'them-ho-so', 'lich-su']),
  'xu-ly':    new Set(['hoan-thanh', 'chuyen-xu-ly', 'tao-vb-di', 'them-ho-so', 'lich-su']),
}
...
function Toolbar({ status, role, onAction }: {
  ...
  const primaryBtn = () => {
    if (status === 'cho-chi-dao' && can('chi-dao'))
      return <button className="dp-btn primary" onClick={() => onAction('chi-dao')}>Chỉ đạo ngay</button>
    if (status === 'cho-dieu-phoi' && can('dieu-phoi'))
      return <button className="dp-btn primary" onClick={() => onAction('dieu-phoi')}>Điều phối ngay</button>
    if (status === 'cho-xu-ly' && can('hoan-thanh'))
      return (
  ...
  const menuItems = () => {
    ...
    return all.filter(item => can(item.key) || item.key === 'tao-vb-di' && can('tao-vb-di'))
  }
  ...
  return (
    <div className="cw" style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <Toolbar status={status} role={currentUser.role} onAction={handleAction} />
