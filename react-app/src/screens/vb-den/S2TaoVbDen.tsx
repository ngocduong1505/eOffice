...
export default function S2TaoVbDen() {
  ...
  const handleSend = () => {
    ...
    goScreen('s1', {
      handoff: {
        msg: 'Đã tiếp nhận văn bản. Gửi lên Giám đốc ghi ý kiến chỉ đạo.',
        nextUserId: 'le-van-gd',
      },
    })
  }
  ...
  return (
    <div className="cw" style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'grid', gridTemplateColumns: '3fr 2fr' }}>
        ...
        {/* ══ CỘT PHẢI: Tệp + PDF Preview ══ */}
        <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#f1f3f7' }}>

          {/* Phần trên — File section */}
          <div style={{ flexShrink: 0, background: '#fff', borderLeft: '1px solid var(--border)' }}>
            {/* Header label */}
            {/* Tab bar */}
                ...
                <>
                  {mainFiles.map((f, i) => (
                    <div
                      ...
                      style={{
                        display: 'flex', alignItems: 'center', gap: 10,
                        border: `1px solid ${selectedPreview?.name === f.name ? 'var(--orange)' : 'var(--border)'}`,
                        borderRadius: 8, padding: '10px 12px',
                        background: selectedPreview?.name === f.name ? '#fff7ed' : '#fafbfc',
                        marginBottom: 6, cursor: 'pointer',
                      }}
                    >
                      <span style={{ fontSize: '1.2rem' }}>{f.type === 'xlsx' ? '📊' : '📄'}</span>
                      <div style={{ flex: 1 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              <span style={{ flex: 1, fontSize: '.8rem', fontWeight: 500, opacity: .9 }}>
                {selectedPreview ? `📄 ${selectedPreview.name}` : 'Chọn file để xem trước'}
              </span>
              {selectedPreview && (
                <>
